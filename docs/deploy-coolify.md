...
| `/me` shows a sign-in panel with no Microsoft button | Interactive login is not configured — sign in by pasting your bearer token instead (your `MCP_TOKENS_*` entry's label is your identity on the page). The Microsoft button and one-click upstream Connect appear once you configure an OIDC issuer plus `AUTH_CLIENT_ID`/`AUTH_CLIENT_SECRET`/`SESSION_SECRET`, see [Standalone with OAuth](standalone-oauth-entra.md). On an OAuth-enabled deploy, `/me?signin=token` is the break-glass path to the token panel. |
